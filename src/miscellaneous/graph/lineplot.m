...
while(c<=length(varargin))
    switch(varargin{c})
        case {'gap','XTickLabel','XCoord','plotfcn','errorfcn','errorfcn_up','errorfcn_down','leg','leglocation','xcat','flip'}
            eval([varargin{c} '=varargin{c+1};']);
            c=c+2;
        % Style tabs: single value sets it for all values, cell array puts
        % in the cat structure 
        case {'markertype','markerfill','linecolor','linewidth','markercolor','markersize','errorcolor','errorwidth','errorbars'}
            v=varargin{c+1}; 
            eval(['F.' varargin{c} '=v;']);  
            c=c+2;
        case 'subset'
    ...
    for seg=1:length(l_from)
        ...
        if (~isempty(errorfcn))
            ...
            if (strcmp(fm.errorbars,'plusminus'))
                errorbars(x,y',[EU' ED'],'linecolor',fm.errorcolor,'linewidth',fm.errorwidth,'error_dir','both');
