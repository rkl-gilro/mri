function [F_obj, J] = myfun_unc_ortho_eval(mesh0_X)
...
tri1 = TriRep( source_tri_v.Triangulation,[mesh11' source_tri_v.X(:,3)]); % define the new mesh for the axial
tri2 = TriRep( source_tri_v.Triangulation,[source_tri_v.X(:,1) mesh21']); % define the new mesh for the sagittal
tri3 = TriRep( source_tri_v.Triangulation,[mesh31(1,:)' source_tri_v.X(:,2) mesh31(2,:)']); % define the new mesh for the sagittal
...
lapl_tri1 = zeros(size( source_tri_v.X,1), xyz);
lapl_tri2 = zeros(size( source_tri_v.X,1), xyz);
lapl_tri3 = zeros(size( source_tri_v.X,1), xyz);
...
%     F_s(i + 2*incr)  =  sqrt_lambda * (sum( (tri3.X(i,:) - mean(tri3.X( list_edges_v{i},:))).^2 ));
    ...
    mean_tmp1 = mean(tri1.X(list_edges_v{i},1:2)); % axial (x,y)
    mean_tmp2 = mean(tri2.X(list_edges_v{i},2:3)); % sagittal (y,z)
    mean_tmp3 = mean([tri3.X(list_edges_v{i},1) tri3.X(list_edges_v{i},3)]); % coronal (x,z)
...
% F_obj = .5 .* (sum(F1.^2)/n1 + sum(F2.^2)/n2 + sum(F3.^2)/n3 + sum(F_s)/(3*size( source_tri_v.X,1)));
F_obj = (.5/(n1 + n2 + n3 + 3*size( source_tri_v.X,1)) ).* (sum(F1.^2) + sum(F2.^2) + sum(F3.^2) + sum(F_s));
...
%         inters = slice_int1(i);
        ...
        ind_w = (tri2.Triangulation(current_tr_int1_v(i),:)-1)*xyz + 1;
        
        % axial (x,y)
        w(1,ind_w ) = c2b_coord_int1_v(i,:); 
        w(2,:)  = circshift(w(1,:)', 1);
        % sagittal (y,z)
...
%         inters = slice_int2(i);
        ...
        ind_w = (tri1.Triangulation(current_tr_int2_v(inters),:)-1) * xyz + 1;
        
        % axial (x,y)
        w( 1,ind_w ) = c2b_coord_int2_v(inters,:);
        w( 2,:) = circshift(w(1,:)', 1);
        % coronal (x,z)
...
%         inters = slice_int3(i);
        ...
        ind_w = (tri3.Triangulation(current_tr_int3_v(inters),:)-1) * xyz + 1;
        
        w_tmp( 1,ind_w ) = c2b_coord_int3_v(inters,:);
    ...
    for i = 1:size( source_tri_v.X,1)
        ...
        for j = 1:length(list_edges_v{i})
            
            edgein1 = (list_edges_v{i}(j)-1) * xyz + 1;
            edgein2 = edgein1 + incr_col;
            edgein3 = edgein2 + incr_col;
            
            Js_s(i   ,         edgein1:edgein1+1) = Js_s(i ,         jsi1:jsi1+1).*[scalar_v(i) scalar_v(i)];
            Js_s(i + incr,     edgein2:edgein2+1) = Js_s(i + incr,   jsi2:jsi2+1).*[scalar_v(i) scalar_v(i)];
            Js_s(i + 2 * incr, edgein3:edgein3+1) = Js_s(i + 2*incr, jsi3:jsi3+1).*[scalar_v(i) scalar_v(i)];
...
%             Js_s(i + 2 * incr, edgein3:edgein3+2) = Js_s(i + 2*incr, jsi3:jsi3+2).*[scalar(i) scalar(i) scalar(i)];
    ...
    J =  (1/(n1 + n2 + n3 + 3*size( source_tri_v.X,1)) ).*(sum(Js_int1) + sum(Js_int2) + sum(Js_int3) + sum(Js_s));
